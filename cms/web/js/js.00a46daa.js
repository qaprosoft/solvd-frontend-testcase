...
function tabsFactory(_ref) {
  var tabItemSelector = _ref.tabItemSelector,
      tabButtonSelector = _ref.tabButtonSelector;
  var ref = {
  ...
  for (refKey in ref) {
    if (ref[refKey].length === 0) {
      console.error("Can't find references for \"".concat(refKey, "\""));
      return;
    }
  }
  ...
  var setActive = function setActive(ref, value) {
    return ref.setAttribute('data-active', value);
  };

  var resetActives = function resetActives() {
    ref.tabs.forEach(function (tab, idx) {
      setActive(tab, 'false');
      setActive(ref.buttons[idx], 'false');
    });
  };

  ref.buttons.forEach(function (r, idx) {
